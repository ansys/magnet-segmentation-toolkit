...
for magnet in magnets:
    ...
    x_pointing = go.distance_vector(
        magnet.top_face_z.center, new_list[0].vertices[0].position, new_list[0].vertices[1].position
    )
    y_pointing = go.distance_vector(
        magnet.top_face_z.center, new_list[2].vertices[0].position, new_list[2].vertices[1].position
    )
    cs = m3d.modeler.create_coordinate_system(
        magnet.top_face_z.center, name=magnet.name + "_cs", x_pointing=x_pointing, y_pointing=y_pointing
    )
