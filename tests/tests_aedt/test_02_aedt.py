...
class TestClass(BasisTest, object):
    def setup_class(self):
        ...
        self.aedtapp_analyzed = BasisTest.add_app(self, application=Maxwell3d, project_name=test_project_name_analyzed)
    ...
    def test_04_mesh(self):
        ...
        windings_mesh = [mesh for mesh in self.aedt.maxwell.mesh.meshoperations if mesh.name == "windings_mesh"]
        assert windings_mesh[0].props["MaxLength"] == "5mm"
        rotor_and_stator_mesh = [
            mesh for mesh in self.aedt.maxwell.mesh.meshoperations if mesh.name == "rotor_and_stator_mesh"
        ]
        assert rotor_and_stator_mesh[0].props["MaxLength"] == "8mm"
        magnets_mesh = [mesh for mesh in self.aedt.maxwell.mesh.meshoperations if mesh.name == "magnets_mesh"]
