...
class AedtFlow(ToolkitGeneric):
    ...
    def apply_skew(self):
        ...
        bound_dep_id = [bound for bound in self.maxwell.boundaries if bound.type == "Dependent"][0].props["Objects"][0]
        ...
        for rotor_object in rotor_objects:
            ...
            for obj in objs_in_bb[rotor_object.name]:
                if obj in magnets:
                    magnet_cs = [
                        cs for cs in self.maxwell.modeler.coordinate_systems if cs.name == obj.part_coordinate_system
                    ][0]
